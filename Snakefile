...
SIMULATION_JSD_RESULT_FILE_BY_PK = j(SIMULATION_RESULT_DIR_BY_PK, "result", "result_{p}_{k}.csv")
SIMULATION_ENTROPY_RESULT_FILE_BY_PK = j(SIMULATION_RESULT_DIR_BY_PK, "entropy", "entropy_{p}_{k}.npy")
SIMULATION_VARIANCE_RESULT_FILE_BY_PK = j(SIMULATION_RESULT_DIR_BY_PK, "variance", "variance_{p}_{k}.npy")


SIMULATION_RESULT_DIR_BY_GAMMA = j(SIMULATION_RESULT_DIR, 'find_gamma')

SIMULATION_JSD_RESULT_FILE_BY_GAMMA = j(SIMULATION_RESULT_DIR_BY_GAMMA, "result", "result_{gamma}.csv")
SIMULATION_ENTROPY_RESULT_FILE_BY_GAMMA = j(SIMULATION_RESULT_DIR_BY_GAMMA, "entropy", "entropy_{gamma}.npy")
SIMULATION_VARIANCE_RESULT_FILE_BY_GAMMA = j(SIMULATION_RESULT_DIR_BY_GAMMA, "variance", "variance_{gamma}.npy")
...
rule calculate_transition_matrix_and_psi:
    ...
    output: hotspot_matrix_fig=HOTSPOT_MATRIX_FIG_BY_PERIOD, null_hotspot_matrix_fig=NULL_HOTSPOT_MATRIX_FIG_BY_PERIOD, ratio_fig=RATIO_FIG_BY_PERIOD, psi_result=PSI_RESULT_BY_PERIOD
...
rule calculate_distance_and_travel_time:
    input: hotspot_data=GRID_HOTSPOT_LEVEL_BY_PERIOD
    output: distance=DISTANCE
    script: "workflow/scripts/calculate_geodistance.py"
...
rule calculate_entropy_variance_data:
    input: sequence=SEQUENCE_BY_PERIOD, vendor_to_grid=VENDOR_TO_GRID_BY_PERIOD, grid_to_hotspot_level=GRID_HOTSPOT_LEVEL_BY_PERIOD, font_file=NORMAL_FONT_PATH, dist=DISTANCE
...
rule simulation_by_p_and_k:
    input: home_pdf=HOME_PDF_BY_PERIOD, sequence_length=SEQUENCE_LENGTH_BY_PERIOD, hotspot_level_by_grid=GRID_HOTSPOT_LEVEL_BY_PERIOD, entropy_data=ENTROPY_DATA_BY_PERIOD, variance_data=VARIANCE_DATA_BY_PERIOD, dist=DISTANCE
    output: jsd=SIMULATION_JSD_RESULT_FILE_BY_PK, entropy=SIMULATION_ENTROPY_RESULT_FILE_BY_PK, variance=SIMULATION_VARIANCE_RESULT_FILE_BY_PK
...
rule draw_simulation_result_by_p_and_k:
    input: before_files=expand(SIMULATION_JSD_RESULT_FILE_BY_PK, period=['before'], p=ps, k=ks), after_files=expand(SIMULATION_JSD_RESULT_FILE_BY_PK, period=['after'], p=ps, k=ks), font_file=NORMAL_FONT_PATH
    output:simulation_fig_by_pk =SIMULATION_FIG_BY_PK
    params: ps=ps, ks=ks
    script: "workflow/scripts/draw_simulation_result_hotspot_entropy.py"

    
rule simulation_by_gamma:
    input: home_pdf=HOME_PDF_BY_PERIOD, sequence_length=SEQUENCE_LENGTH_BY_PERIOD, hotspot_level_by_grid=GRID_HOTSPOT_LEVEL_BY_PERIOD, entropy_data=ENTROPY_DATA_BY_PERIOD, variance_data=VARIANCE_DATA_BY_PERIOD, dist=DISTANCE
    output: jsd=SIMULATION_JSD_RESULT_FILE_BY_GAMMA, entropy=SIMULATION_ENTROPY_RESULT_FILE_BY_GAMMA, variance=SIMULATION_VARIANCE_RESULT_FILE_BY_GAMMA
    params: hotspot_level=HOTSPOT_LEVEL, max_entropy=MAX_ENTROPY, max_variance=MAX_VARIANCE, n_entropy_bin=N_ENTROPY_BIN, n_variance_bin=N_VARIANCE_BIN, repetition=REPTITION
    script: "workflow/scripts/simulation_find_gamma.py"
    
    
rule draw_simulation_result_by_gamma:
    input: before_files=expand(SIMULATION_JSD_RESULT_FILE_BY_GAMMA, period=['before'], gamma=gammas), after_files=expand(SIMULATION_JSD_RESULT_FILE_BY_GAMMA, period=['after'], gamma=gammas), font_file=NORMAL_FONT_PATH
    output:simulation_fig_by_gamma =SIMULATION_FIG_BY_GAMMA
    params: gammas=gammas
    script: "workflow/scripts/draw_simulation_result_locational_variance.py"
